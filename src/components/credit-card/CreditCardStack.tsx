...
export const CreditCardStack = () => {
  ...
  return (
    <div 
      ref={containerRef}
      className="relative h-48 w-full select-none"
    ...
    >
      {creditCards.map((card, index) => (
        <div
          key={card.id}
          className={cn(
            "absolute left-0 right-0 w-full h-40 rounded-apple p-6 cursor-pointer",
            ...
            index === activeIndex + 1 ? 
              "z-20 translate-y-4 opacity-90 scale-[0.95] blur-[1px]" :
            index === activeIndex + 2 ? 
              "z-10 translate-y-8 opacity-80 scale-[0.9] blur-[2px]" :
            index > activeIndex + 2 ?
              "opacity-0 translate-y-12 scale-[0.85]" :
            index < activeIndex ?
              "opacity-0 -translate-y-4 scale-[0.85]" : ""
          )}
